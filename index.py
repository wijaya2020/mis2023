...
def index():
    ...
    homepage = "<center><h1>資訊管理導論 - 期末報告</h1>"
    homepage += "<h2>授課教師 : 楊子青 老師</h2>"
    
    homepage += "<h2>專案名稱 :<br>"
    homepage += "加密貨幣 (Python+Flask+Firebase+Linebot+Dialoglow)<br>"
    homepage += "<img src=\"/static/images/crypto.jpg\" alt=\"照片\"><br>"
    homepage += "Bot ID : @340afcat</h2><br>"

    homepage += "<h2>組員 : <br>"
    homepage += "410870419 詹忻諭<br>" 
    homepage += "410930201 魏清昊</h2><br><br>"
    
    homepage += "<h3><a href=/crypto>crypto listing price </a><br>"
    homepage += "<a href=/cryptoUpload>upload data crypto listing price to Firestore</a><br>"
    homepage += "<a href=/cryptosearch>輸入加密貨幣符號，進行搜尋</a></h3><br><br><br><br><br>"
    
    homepage += "<h5>Copyright © 2022. All Rights Reserved.</center></h5><br>"
...
def cryptosearch():
    if request.method == "POST":
        ...
        #docs = collection_ref.where("title","==", "濁水漂流").get()
        docs = collection_ref.order_by("Rank2").get()
        ...
        for doc in docs:
            if CryptoSymbol.upper() in doc.to_dict()["Symbol"]:
                ...
                #get data
                cmc_rank = str(doc.to_dict()["Rank2"])
                name = doc.to_dict()["Name2"]
...
def handle_message(event):
    ...
    #Z or ? - help  (how to type keyword) 
        ...
        info += "\n\n" 
        info += "Type: \"LATEST <coin symbol>\" --> for a coin latest price." 
        info += "\n" 
        info += "e.g.: \"LATEST BTC\"" 
        info += "\n\n" 
        info += "Type: \"RANK\" --> for Top 10 coin latest price." 
        info += "\n\n"    
        info += "Type: \"GAINNERS\" --> for Top 10 coin gainners 24h chg%." 
        info += "\n\n" 
        info += "Type: \"LOSERS\" --> for Top 10 coin losers 24h chg%." 
        info += "\n\n" 
        info += "Type: \"VOL\" --> for Top 10 coin 24h vol chg%." 
        info += "\n\n" 
        info += "Type: \"FAV\" --> for My favourite's coin." 
        info += "\n\n" 
        info += "Type: \"LIST\" --> for List of all coin's names & symbols" 
        info += "\n\n" 
        info += "Type: \"TERMS\" --> for cypto terms."
        info += "\n\n" 
        info += "Type: \"HELP\" or \"?\" --> for help how to type search keyword." 
        info += "\n\n" 
...
def searchCryptoRANK(keyword):
    ...
    info = "Latest Price Top 10 Coin by Rank：" + keyword + "\n\n"    
    
    collection_ref = db.collection("cryptocurrency")
    docs = collection_ref.order_by("Rank2").limit(10).get()

    for doc in docs:
        #get data        
        cmc_rank = doc.to_dict()["Rank2"]
        name = doc.to_dict()["Name2"]
        symbol = doc.to_dict()["Symbol"]
        price = doc.to_dict()["Price"]
        percent_change_24h = doc.to_dict()["Percent_Change_24h"] 
...
def searchCryptoLATEST(keyword):
    ...
    collection_ref = db.collection("cryptocurrency")
    docs = collection_ref.order_by("Rank2").limit(108).get()
    found = False
...
def webhook():
    ...
    if (action == "CityWeather"):
    ...
    elif (action == "actCoinSymbol"):
        message =  req.get("queryResult").get("parameters").get("coin")
        #info = "動作：" + action + "； 查詢內容：" + message + "\n"
        #return make_response(jsonify({"fulfillmentText": info}))
        #info = message
        
        doc_ref = db.document("cryptocurrency/" + message)
        doc = doc_ref.get()
        result = doc.to_dict()

        cmc_rank = result.get("Rank2") 
        name = result.get("Name2") 
        symbol = result.get("Symbol") 
        price = result.get("Price") 
        percent_change_24h =  result.get("Percent_Change_24h") 
        percent_change_7d =  result.get("Percent_Change_7d") 
        market_cap_dominance =  result.get("Market Cap Dominance")  
        ...
        return make_response(
            jsonify({
                ...
                "fulfillmentMessages": [
                    {"quickReplies": {
                        "title": info,
                        "quickReplies": ["latest btc", "虛擬貨幣 ETH"]
                }}]
            })
        )

    elif (action == "actCoinQ"):
        cond =  req.get("queryResult").get("parameters").get("coinQ")
        keyword =  req.get("queryResult").get("parameters").get("any")
        #info = "you ask me to find " + cond + " price for coin symbol : " + keyword + "\n\n" 
        #return make_response(jsonify({"fulfillmentText": info}))
        
        info = "Latest Price：" + "\n\n"
        
        collection_ref = db.collection("cryptocurrency")
        docs = collection_ref.order_by("Symbol").get()
        found = False

        for doc in docs:
            if str(keyword).upper() in doc.to_dict()["Symbol"]:
                found = True
                #get data        
                cmc_rank = doc.to_dict()["Rank2"]
                name = doc.to_dict()["Name2"]
        ...
        doc = doc_ref.get()
        result = doc.to_dict()
        timestampDate = str(result.get("timestampDate"))
        timestampTime = str(result.get("timestampTime"))
    ...
    elif (action == "actCoinInfo"):
        message =  req.get("queryResult").get("parameters").get("CoinInfo")
        
        #ALL - latest price
        if (message == 'RANK'):
            #info = "動作：" + action + "； 查詢內容：" + message + "\n"
            #return make_response(jsonify({"fulfillmentText": info}))
            info = "Latest Price Top 10 Coin by Rank：" + "\n\n"
            ...
            return make_response(
                jsonify({
                    ...
                    "fulfillmentMessages": [
                        {"quickReplies": {
                            "title": info,
                            "quickReplies": ["info Gainners", "info Losers", "info Vol"]
                    }}]
                })
            )

        elif (message == 'GAINNERS'):
            #info = "動作：" + action + "； 查詢內容：" + message + "\n"
            #return make_response(jsonify({"fulfillmentText": info}))
            ...
            info += "e.g.: \"LATEST BTC\"" 
            info += "\n\n" 
            info += "Type: \"info RANK\" --> for Top 10 coin latest price." 
            info += "\n\n"    
            info += "Type: \"info GAINNERS\" --> for Top 10 coin gainners 24h chg%." 
            info += "\n\n" 
            info += "Type: \"info LOSERS\" --> for Top 10 coin losers 24h chg%." 
            info += "\n\n" 
            info += "Type: \"info VOL\" --> for Top 10 coin 24h vol chg%." 
            info += "\n\n" 
            info += "Type: \"info FAV\" --> for My favourite's coin." 
            info += "\n\n" 
            info += "Type: \"info LIST\" --> for List of all coin's names & symbols" 
            info += "\n\n" 
            info += "Type: \"info TERMS\" --> for cypto terms."
            info += "\n\n" 
            info += "Type: \"info HELP\" or \"?\" --> for help how to type search keyword." 
            info += "\n\n" 
